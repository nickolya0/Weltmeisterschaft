...
class Tabelle:
    ...
    def initializeTabelle(self, root):
        ...
        for i in self.scoreList:
            tv.insert(parent='', index=nummer, iid=nummer, text='', values=(self.manschaftList_Sort[nummer], self.scoreList[nummer].split(' ')[1], self.toreZusamenVerbinden[nummer].split(' ')[1]))
            nummer += 1
        tv.pack()
